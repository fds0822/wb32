...
bool usb_host_indicator_enable  = false;
...
__attribute__((weak)) void usb_transport_enable(bool enable) {
    ...
    if (enable) {
		...
		//if( (host_get_driver() != &chibios_driver)||!usb_power_connected() ){
			//LOG_TRACE("e0 \n\r"); 
...
void set_transport(transport_t new_transport) {
    ...
    //LOG_TRACE("set_transport: %s, bt statue: %s\n\r", xport[new_transport], bt_state_list[wireless_get_state()]);
#endif

    if (transport != new_transport) {
             
        if (transport == TRANSPORT_USB || 
          ((transport != TRANSPORT_USB) && wireless_get_state()  == WT_CONNECTED))
            clear_keyboard();
    
        transport = new_transport;
		LOG_TRACE("transport:%d\n\r",transport); 
...
void usb_remote_wakeup(void) {
    if (USB_DRIVER.state == USB_SUSPENDED) {
		wait_ms(300);
		if(usb_power_connected()){
			while (USB_DRIVER.state == USB_SUSPENDED) {LOG_TRACE("wt-urw-SUSPENDED\n\r");
