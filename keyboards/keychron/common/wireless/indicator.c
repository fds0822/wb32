...
static uint8_t usb_host_led_matrix_list[USB_HOST_DEVICES_COUNT] = USB_HOST_LED_MATRIX_LIST;
#    endif

#    ifdef OsSwitch_LED_MATRIX_LIST
static uint8_t osswithc_led_matrix_list[OsSwitch_LED_COUNT] = OsSwitch_LED_MATRIX_LIST;
#    endif

#    ifdef WinLock_LED_MATRIX_LIST
static uint8_t winlock_led_matrix_list[WinLock_LED_COUNT] = WinLock_LED_MATRIX_LIST;
#    endif
...
#ifdef USB_HOST_LED_MATRIX_LIST
uint8_t indicator_usb_host_count  = 0;
extern bool usb_host_indicator_enable;
#    define SET_LED_USB_HOST(idx) rgb_matrix_set_color(idx, 0, 0, 255)
# define indicator_usb_host_number   3
# define indicator_usb_host_interval 100 //ms
# define indicator_usb_host_connected 2000 //ms
#endif

#ifdef	OsSwitch_LED_MATRIX_LIST
extern bool ossw_indicator_enable;
#    define SET_LED_OSSW_WIN(idx) rgb_matrix_set_color(idx, 0, 0, 255)
#    define SET_LED_OSSW_MAC(idx) rgb_matrix_set_color(idx, 0, 255, 0)
# define set_led_status_unknown  0
# define set_led_status_all_led  1
# define set_led_status_led      2
# define indicator_ossw_number   3
# define indicator_ossw_interval 300 //ms
#endif
...
extern  wt_state_t wireless_state;
extern  bool User_get_evtsleep;
#endif
...
void indicator_init(void) {
	...
	LOG_TRACE("LEMOKEY_CALLBACK_ENABLE \n\r");
...
void indicator_set(wt_state_t state, uint8_t host_index) {
    ...
    //LOG_TRACE("indicator set [%s] to [%s], idx:%d\n\r", bt_state_list[current_state], bt_state_list[state], host_index);
...
#if 1
	//LOG_TRACE("battery_low_backlit_enable 1->"); 

    if (enable) {
        uint32_t t = rtc_timer_read_ms();
		//LOG_TRACE("battery_low_backlit_enable2 %d\n\r",t);
	    //LOG_TRACE("rtc_time1 %d\n\r",rtc_time);
        /* Check overflow */
        if (rtc_time > t) {
        ...
        }//LOG_TRACE("rtc_time2 %d\n\r",rtc_time);
        /* Indicating at first time or after the interval */
        if ((rtc_time == 0 || t - rtc_time > LOW_BAT_LED_TRIG_INTERVAL) && bat_low_ind_state == 0) {
            bat_low_backlit_indicator = enable ? timer_read32() : 0;//LOG_TRACE("battery_low_backlit_enable 4->");
            rtc_time                  = rtc_timer_read_ms();
            bat_low_ind_state         = 1;

            indicator_enable();
        }
    } else {//LOG_TRACE("battery_low_backlit_enable 3->");
        rtc_time          = 0;
        bat_low_ind_state = 0;
...
void indicator_usb_host(void){
	static uint32_t usb_host_indicator_timer   = 0;
	//static uint8_t indicator_usb_host_count  = 0;
	
	if(usb_host_indicator_enable){
		if(usb_host_indicator_timer==0){
			usb_host_indicator_timer=timer_read32();	
			indicator_enable();
		    indicator_reset_backlit_time();
		}
		SET_ALL_LED_OFF();

		if (usb_host_indicator_timer && timer_elapsed32(usb_host_indicator_timer) > indicator_usb_host_interval) {
			usb_host_indicator_timer=0;
			if(indicator_usb_host_count < indicator_usb_host_number*2){
					indicator_usb_host_count++;// LOG_TRACE("indicator_usb_host_count11: %d\n\r",indicator_usb_host_count);
				}
			else{
					usb_host_indicator_enable=false;//LOG_TRACE("indicator_usb_host_count00: %d\n\r",indicator_usb_host_count);
		...
		if(indicator_usb_host_count % 2){
				SET_LED_USB_HOST(usb_host_led_matrix_list[USB_HOST_DEVICES_COUNT-1]);			
			}
		else{					
				SET_LED_OFF(usb_host_led_matrix_list[USB_HOST_DEVICES_COUNT-1]);				
			}		
	}

	if(indicator_usb_host_count==indicator_usb_host_number*2 && !usb_host_indicator_enable && USBD1.state==USB_ACTIVE){
		if(usb_host_indicator_timer==0){
			usb_host_indicator_timer=timer_read32();	
			indicator_enable();
			indicator_reset_backlit_time();
		...
		SET_ALL_LED_OFF();
		SET_LED_USB_HOST(usb_host_led_matrix_list[USB_HOST_DEVICES_COUNT-1]);
		if (usb_host_indicator_timer && timer_elapsed32(usb_host_indicator_timer) > indicator_usb_host_connected) {
				usb_host_indicator_timer=0;
	...
	if (get_transport() != TRANSPORT_USB){
		usb_host_indicator_enable=false;
		usb_host_indicator_timer=0;
		indicator_usb_host_count=0;
		}
//	LOG_TRACE("Ustate:%d ->",USBD1.state); 
}

#ifdef Custom_Switch
void indicator_ossw(void){
	static uint32_t ossw_indicator_timer   = 0;
	static uint8_t indicator_ossw_count = 0;
	static uint8_t user_set_led_status = 0;

	if(ossw_indicator_enable){

		if(user_set_led_status==set_led_status_unknown){
			if (!LED_DRIVER_IS_ENABLED()) {
				indicator_enable();
			    indicator_reset_backlit_time();
				user_set_led_status=set_led_status_all_led;
			}
			else {
				user_set_led_status=set_led_status_led;
			}
		}

		if(user_set_led_status==set_led_status_all_led){
			SET_ALL_LED_OFF();
		}
		else if(user_set_led_status==set_led_status_led){
			SET_LED_OFF(osswithc_led_matrix_list[OsSwitch_LED_COUNT-1]);
		}


		if(ossw_indicator_timer==0){
			ossw_indicator_timer=timer_read32();			
		}

		if (ossw_indicator_timer && timer_elapsed32(ossw_indicator_timer) > indicator_ossw_interval) {
			ossw_indicator_timer=0;
			if(indicator_ossw_count < indicator_ossw_number*2){
					indicator_ossw_count++;	//LOG_TRACE("indicator_ossw_count11: %d\n\r",indicator_ossw_count);
				}
			else{
					ossw_indicator_enable=false;//LOG_TRACE("indicator_ossw_count00: %d\n\r",indicator_ossw_count);
					indicator_ossw_count=0;
					user_set_led_status=set_led_status_unknown;
					indicator_stop();					
				}
			}		

		
		if(indicator_ossw_count % 2){
			if(default_layer_state == 1U << 0)
			{
				SET_LED_OSSW_MAC(osswithc_led_matrix_list[OsSwitch_LED_COUNT-1]);	
			}
			else if(default_layer_state == 1U << 2)
			{
				SET_LED_OSSW_WIN(osswithc_led_matrix_list[OsSwitch_LED_COUNT-1]);	
			}
		}
		else{					
			SET_LED_OFF(osswithc_led_matrix_list[OsSwitch_LED_COUNT-1]);				
...
#    endif
		...
		if(keymap_config.no_gui){
			SET_LED_WINLOCK(winlock_led_matrix_list[WinLock_LED_COUNT-1]);
		}
#endif
#ifdef	WIN_LOCK_LED_PIN
		if(keymap_config.no_gui){
			if( (default_layer_state == 1U << 2) && (User_get_evtsleep==false) && ((wireless_state == WT_CONNECTED) || (get_transport() == TRANSPORT_USB&&USBD1.state==USB_ACTIVE)) ){
#        if defined(DIM_WIN_LOCK)
        		SET_LED_OFF(WIN_LOCK_INDEX);//LOG_TRACE("osi 3");
#        else
        		SET_LED_ON(WIN_LOCK_INDEX);
#        endif
				writePin(WIN_LOCK_LED_PIN, keymap_config.no_gui);
			}
			else{
...
#    endif
        ...
        if (indicator_config.value) {
            ...
            if (indicator_config.highlight) {
                SET_ALL_LED_OFF();//LOG_TRACE("SET_ALL_LED_OFF \n\r");	
            ...
            if (indicator_config.value & LED_ON) {
#    ifdef P2P4G_HOST_LED_MATRIX_LIST
				//LOG_TRACE("indicator_config.value & LED_ON \n\r");	
...
#    ifdef P2P4G_HOST_LED_MATRIX_LIST
				//LOG_TRACE("indicator_config.value & LED_ON else \n\r");		
...
bool led_update_kb(led_t led_state) {//LOG_TRACE("led_update_kb1 ");	
    bool res = led_update_user(led_state);
    if (res) {//LOG_TRACE("led_update_kb2 ");	
...
#    endif 
            SET_ALL_LED_OFF();//LOG_TRACE("led_update_kb3 ");
            os_state_indicate();
            LED_DRIVER.flush();
#    if defined(LED_MATRIX_DRIVER_SHUTDOWN_ENABLE) || defined(RGB_MATRIX_DRIVER_SHUTDOWN_ENABLE)
