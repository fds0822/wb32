...
extern void lowpower_before_config(void);
...
bool pre_enter_low_power_mode(pm_t mode) {
    ...
    // PWR->CR2 &= ~PWR_CR2_USV; /*PWR_CR2_USV is available on WB32L4x2xx and WB32L4x3xx devices only. */
#endif
...
#else
	LOG_TRACE("Enable key matrix wake up \r\n");
...
#endif    
	palSetLineMode(A12, PAL_MODE_INPUT_PULLUP);
...
#    define NUMBER_OF_DIP_SWITCHES (sizeof(dip_switch_pad) / sizeof(pin_t))
	...
	palSetLineMode(D0, PAL_MODE_INPUT_PULLDOWN);
	palSetLineMode(D1, PAL_MODE_INPUT_PULLDOWN);

	//palSetLineMode(C13, PAL_MODE_INPUT_PULLDOWN);
	palSetLineMode(C14, PAL_MODE_INPUT_PULLDOWN);
	palSetLineMode(C11, PAL_MODE_INPUT_PULLDOWN);
	palSetLineMode(B7, PAL_MODE_INPUT_PULLDOWN);

	lowpower_before_config();
    return true;
}
...
static inline void lpm_wakeup(void) {
    ...
    // timer_init();
    palSetLineMode(A11,
                    PAL_WB32_OTYPE_PUSHPULL | PAL_WB32_OSPEED_HIGH
                    | PAL_WB32_PUPDR_FLOATING  | PAL_MODE_ALTERNATE(10U));
    palSetLineMode(A12,
                    PAL_WB32_OTYPE_PUSHPULL | PAL_WB32_OSPEED_HIGH
                    | PAL_WB32_PUPDR_FLOATING  | PAL_MODE_ALTERNATE(10U));
...
#    if defined(KEEP_USB_CONNECTION_IN_WIRELESS_MODE)
    ...
    if (usb_power_connected() && (get_transport() == TRANSPORT_USB)) {
...
//LOG_TRACE(".");
        if ((get_transport() == TRANSPORT_BLUETOOTH || get_transport() == TRANSPORT_P2P4 )
           ...
           //!rgb_matrix_is_enabled () || (rgb_matrix_is_enabled() && rgb_matrix_is_driver_shutdown())
            ...
            {
                 //LOG_TRACE("2 ");
                 LOG_TRACE("2: %d, %d\n\r", !lpm_any_matrix_action() , !battery_power_on_sample());
                 //if ( !lpm_any_matrix_action() && !battery_power_on_sample()) {
                 if ( !lpm_any_matrix_action() ) {
                    LOG_TRACE("3 ");
                    if (pre_enter_low_power_mode(LOW_POWER_MODE)) {//LOW_POWER_MODE PM_SLEEP
	                    enter_power_mode(LOW_POWER_MODE);	
