...
/* LKBT51 disable its uart peripheral to save power if uart inactivity for 3s, need to
 * assert this pin and wait some time for its uart getting ready before sending  data*/
#define LKBT51_WAKE_WAIT_TIME 2000  // us
...
bool User_get_evtsleep=false;
#endif

void lkbt51_init(bool wakeup_from_low_power_mode) {
    //LOG_TRACE("lkbt51_init\n\r");
...
#if (HAL_USE_SPI == TRUE)
    if (BT_DRIVER.state == SPI_UNINIT) {
        setPinOutput(SPI_SCK_PIN);
        writePinHigh(SPI_SCK_PIN);

        palSetLineMode(SPI_SCK_PIN, PAL_MODE_ALTERNATE(SPI_SCK_PAL_MODE));
...
void lkbt51_send_protocol_ver(uint16_t ver) {
        ...
        spiStart(&BT_DRIVER, &spicfg);
        spiSelect(&BT_DRIVER);
        spiSend(&BT_DRIVER, i, pkt);
        spiUnselectI(&BT_DRIVER);
        spiStop(&BT_DRIVER);
#endif
}
...
#if 0
    writePinLow(LKBT51_INT_INPUT_PIN);
    uint32_t wake_time = LKBT51_WAKE_WAIT_TIME;
    if (payload[0] == LKBT51_CMD_PAIRING || payload[0] == LKBT51_CMD_CONNECT )
        wake_time = 50000;
    systime_t start = chVTGetSystemTime();
    while (chTimeI2US(chVTTimeElapsedSinceX(start)) < wake_time ) {};
    writePinHigh(LKBT51_INT_INPUT_PIN);
#endif
    uint16_t checksum = 0;
...
void lkbt51_read(uint8_t* payload, uint8_t len) {
    ...
    spiStart(&BT_DRIVER, &spicfg);
    spiSelect(&BT_DRIVER);
    spiExchange(&BT_DRIVER, i, pkt, payload);
    spiUnselect(&BT_DRIVER);
    spiStop(&BT_DRIVER);
#endif
}
...
void lkbt51_send_system(uint16_t report) {
    ...
    if (hid_usage < 0x81 || hid_usage > 0x83)
        return;
...
void lkbt51_factory_reset(void) {
    uint8_t i = 0;
    memset(payload, 0, PACKET_MAX_LEN);
...
//        //LOG_TRACE("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
//            buf[4], buf[5], buf[6], buf[7], buf[8], buf[9], buf[10], buf[11], buf[12], buf[13], buf[14], buf[15]);
...
//        );
        ...
        if (pbuf[0] == 0xAA && pbuf[1] == 0x54 && pbuf[4] == (uint8_t)(~0x54) && pbuf[5] == (uint8_t)(~0xAA) ) {
            ...
            //LOG_TRACE("protol_ver: %x\n\r", protol_ver);
            (void)protol_ver;
        }
        else if (pbuf[0] == 0xAA) {
            wireless_event_t event = {0};
            //LOG_TRACE("status:%x %x\n", buf[5], buf[6] );
            ...
            if (evt_mask & LK_EVT_MSK_CONNECTION) {
                ...
                switch (pbuf[2]) {
                    ...
                    case LKBT51_DISCONNECTED:
                        event.evt_type = EVT_DISCONNECTED;LOG_TRACE("EVT_DISCONNECTED ");
                        break;
            ...
            if (evt_mask & LK_EVT_MSK_LED) {
                ...
                event.evt_type   = EVT_HID_INDICATOR;
                event.params.led = pbuf[4];LOG_TRACE("event.params.led ");
                wireless_event_enqueue(event);
            }
            ...
            if (evt_mask & LK_EVT_MSK_RPT_INTERVAL) {
                ...
                ////LOG_TRACE("interval: %d.%d ms\n\r", interval/1000, (interval%1000)/10); 
        ...
        if (!wait_for_new_pkt && BUFFER_SIZE-5 >= len) {
            ...
            uint16_t checksum = 0;
            for (int i = 0; i < len - 2; i++) { checksum += pbuf[i]; }
...
#else
  if (0) {lkbt51_event_handler(1, NULL, 0, 0);}
#endif
