...
wt_state_t wireless_state                  = WT_RESET;
...
void wireless_connect_ex(uint8_t host_idx, uint16_t timeout) {
    //LOG_TRACE("wireless_connect_ex %d\n\r", host_idx);
    if (battery_is_critical_low()) return;
...
static void wireless_enter_reset(uint8_t reason) {
    //LOG_TRACE("wireless_enter_reset\n\r");
...
static void wireless_enter_discoverable(uint8_t host_idx) {
    //LOG_TRACE("wireless_enter_discoverable: %d\n\r", host_idx);
    host_index = host_idx;
...
static void wireless_enter_reconnecting(uint8_t host_idx) {
    ...
    //LOG_TRACE("wireless_reconnecting %d\n\r", host_idx);
    wireless_state = WT_RECONNECTING;
    indicator_set(wireless_state, host_idx);
...
static void wireless_enter_connected(uint8_t host_idx) {
    //LOG_TRACE("wireless_connected %d\n\r", host_idx);
...
#endif
    if (wireless_transport.update_bat_level)
        wireless_transport.update_bat_level(battery_get_percentage());
}
...
static void wireless_enter_disconnected(uint8_t host_idx) {
    LOG_TRACE("wireless_disconnected--------------------------------- %d\n\r", host_idx);
...
static void wireless_enter_sleep(void) {
    //LOG_TRACE("wireless_enter_sleep %d\n\r", wireless_state);
    
    led_state = 0;
    if (wireless_state == WT_PARING) {
        wireless_state = WT_SUSPEND;
        //LOG_TRACE("WT_SUSPEND\n\r");
...
#    if defined(NKRO_ENABLE)
        if (wireless_report_protocol && keymap_config.nkro) {
...
void wireless_task(void) {
	//LOG_TRACE("t1"); 
    wireless_transport.task();	//LOG_TRACE("t2"); 
    wireless_event_task();	//LOG_TRACE("t3"); 
#ifndef DISABLE_REPORT_BUFFER
    report_buffer_task();	//LOG_TRACE("t4"); 
#endif
    indicator_task();	//LOG_TRACE("t5"); 
    lemokey_wireless_common_task();	//LOG_TRACE("t6"); 
    battery_task();	//LOG_TRACE("t7"); 
    lpm_task();	//LOG_TRACE("t8"); 
}
