...
action_t action_for_key(uint8_t layer, keypos_t key) {LOG_TRACE("afk-1 ");
    // 16bit keycodes - important
    uint16_t keycode = keymap_key_to_keycode(layer, key);LOG_TRACE("afk-2 ");
    return action_for_keycode(keycode);
};

action_t action_for_keycode(uint16_t keycode) {LOG_TRACE("afkc-1 ");
    // keycode remapping
...
__attribute__((weak)) uint16_t keymap_key_to_keycode(uint8_t layer, keypos_t key) {	
    ...
    if (key.row < MATRIX_ROWS && key.col < MATRIX_COLS) {
		LOG_TRACE("KKTK-1:%X:%X:%X ",layer,key.row ,key.col);
		//LOG_TRACE("KKTK-1T:%X:%X:%X ",layerTemp,rowTemp ,colTemp);
		if( (layer!=layerTemp) || (key.row!=rowTemp) || (key.col!=colTemp) )
		{   
			layerTemp=layer;rowTemp=key.row;colTemp=key.col;
			keycodeTemp=keycode_at_keymap_location(layer, key.row, key.col);
		}
		LOG_TRACE("KKTK-3:%04X ",keycodeTemp);
		return keycodeTemp;
