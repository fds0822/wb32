 ...
 * FIXME: Needs doc
 */
void action_tapping_process(keyrecord_t record) {LOG_TRACE("ATP-1 ");
    if (process_tapping(&record)) {LOG_TRACE("ATP-2");
...
bool process_tapping(keyrecord_t *keyp) {LOG_TRACE("PT-1 ");
    keyevent_t event = keyp->event;
    TAP_DEFINE_KEYCODE;

    // if tapping
    if (IS_TAPPING_PRESSED()) {LOG_TRACE("PT-2 ");
...
#    endif
    ...
    } else if (IS_TAPPING_RELEASED()) {LOG_TRACE("PT-3 ");
        if (WITHIN_TAPPING_TERM(event) || MAYBE_RETRO_SHIFTING(event)) {
            if (event.pressed) {
    ...
    // not tapping state
        ...
        if (event.pressed && is_tap_record(keyp)) {
            ...
            process_record(keyp);LOG_TRACE("PT-7 ");
            return true;
        }
    }
}
