...
void eeconfig_init_quantum(void) {
	...
	eeprom_update_byte(EECONFIG_DEFAULT_MODE, 0);
...
/** \brief eeconfig read default mode
 *
 * FIXME: needs doc
 */
uint8_t eeconfig_read_default_mode(void) {
    return eeprom_read_byte(EECONFIG_DEFAULT_MODE);
}
/** \brief eeconfig update default mode
 *
 * FIXME: needs doc
 */
void eeconfig_update_default_mode(uint8_t val) {
    eeprom_update_byte(EECONFIG_DEFAULT_MODE, val);
}
