...
uint16_t dynamic_keymap_get_keycode(uint8_t layer, uint8_t row, uint8_t column) {
	...
	LOG_TRACE("dkgk-1 ");
    void *address = dynamic_keymap_key_to_eeprom_address(layer, row, column);LOG_TRACE("dkgk-2 ");
    // Big endian, so we can read/write EEPROM directly from host if we want
	/*uint16_t keycode = eeprom_read_byte(address) << 8;//LOG_TRACE("dkgk-3 ");
    keycode |= eeprom_read_byte(address + 1);//LOG_TRACE("dkgk-4 ");*/
	keycode_temp=eeprom_read_word(address);
	uint16_t keycode=((keycode_temp&0XFF)<<8)|((keycode_temp>>8)&0XFF);	LOG_TRACE("dkgk-4:%04x ",keycode);
    return keycode;
}
