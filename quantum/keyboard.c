...
void quantum_init(void) {
    magic();LOG_TRACE("quantum_init:magic\n\r");
    led_init_ports();
#ifdef BACKLIGHT_ENABLE
...
static bool matrix_task(void) {
    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        ...
        for (uint8_t col = 0; col < MATRIX_COLS; col++, col_mask <<= 1) {LOG_TRACE("MT-4 \n\r");
            if (row_changes & col_mask) {
                const bool key_pressed = current_row & col_mask;

                if (process_keypress) {
                    action_exec(MAKE_KEYEVENT(row, col, key_pressed));LOG_TRACE("MT-5 \n\r");
                }
...
void keyboard_task(void) {//LOG_TRACE("kt-1\n\r");
    __attribute__((unused)) bool activity_has_occurred = false;
    if (matrix_task()) {//LOG_TRACE("kt-2\n\r");
